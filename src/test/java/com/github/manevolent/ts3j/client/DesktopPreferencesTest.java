...
public class DesktopPreferencesTest {
    ...
    public void appPreferencesUseTrayAsTheDefaultCloseBehavior() throws Exception {
        ...
        try {
            ...
            store.setCaptureDevice("capture-device-id");
            store.setPlaybackDevice("playback-device-id");
            ...
            assertEquals("capture-device-id", store.captureDevice());
            assertEquals("playback-device-id", store.playbackDevice());
            AppPreferences reloaded = new AppPreferences(preferences);
            assertEquals(37, reloaded.voiceNotificationVolume());
            assertEquals("capture-device-id", reloaded.captureDevice());
            assertEquals("playback-device-id", reloaded.playbackDevice());
